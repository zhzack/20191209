...
const u8* wifiap_ssid=(u8 *)"WildWolf";			//对外SSID号
...
u8 atk_8266_apsta_check(void)
{
	...
	if(atk_8266_check_cmd((u8 *)"+CIPSTATUS:0")&&
		 atk_8266_check_cmd((u8 *)"+CIPSTATUS:1")&&
		 atk_8266_check_cmd((u8 *)"+CIPSTATUS:2")&&
		 atk_8266_check_cmd((u8 *)"+CIPSTATUS:4"))
		return 0;
	else return 1;
}
...
u8 atk_8266_consta_check(void)
{
	...
	if(atk_8266_quit_trans())return 0;			//退出透传 
	atk_8266_send_cmd((u8 *)"AT+CIPSTATUS",(u8 *)":",50);	//发送AT+CIPSTATUS指令,查询连接状态
	p=atk_8266_check_cmd((u8 *)"+CIPSTATUS:"); 
	res=*p;									//得到连接状态	
	return res;
}
//键盘码表
const u8* kbd_tbl[13]={(u8 *)"1",(u8 *)"2",(u8 *)"3",(u8 *)"4",(u8 *)"5",(u8 *)"6",(u8 *)"7",(u8 *)"8",(u8 *)"9",(u8 *)".",(u8 *)"0",(u8 *)"#",(u8 *)"DEL"}; 
u8* kbd_fn_tbl[2];
//加载键盘界面（尺寸为240*140）
...
void atk_8266_msg_show(u16 x,u16 y,u8 wanip)
{
	...
	p2=mymalloc(SRAMIN,32);							//申请32字节内存
	POINT_COLOR=BLUE;
	atk_8266_send_cmd((u8 *)"AT+CWMODE=2",(u8 *)"OK",20);
	...
	atk_8266_send_cmd(p,(u8 *)"OK",1000);
	if(wanip==0)//全更新
	{
		atk_8266_send_cmd((u8 *)"AT+GMR",(u8 *)"OK",20);		//获取固件版本号
		p=atk_8266_check_cmd((u8 *)"SDK version:");
		p1=(u8*)strstr((const char*)(p+12),"(");
		*p1=0;
		Show_Str(x,y,240,16,(u8 *)"固件版本:",16,0);
		Show_Str(x+72,y,240,16,p+12,16,0);
		atk_8266_send_cmd((u8 *)"AT+CWMODE?",(u8 *)"+CWMODE:",20);	//获取网络模式
		p=atk_8266_check_cmd((u8 *)":");
		Show_Str(x,y+16,240,16,(u8 *)"网络模式:",16,0);
		Show_Str(x+72,y+16,240,16,(u8*)ATK_ESP8266_CWMODE_TBL[*(p+1)-'1'],16,0);
  	atk_8266_send_cmd((u8 *)"AT+CWSAP?",(u8 *)"+CWSAP:",20);	//获取wifi配置
		p=atk_8266_check_cmd((u8 *)"\"");
		p1=(u8*)strstr((const char*)(p+1),"\"");
		p2=p1;
		*p1=0;
		Show_Str(x,y+32,240,16,(u8 *)"SSID号:",16,0);Show_Str(x+56,y+32,240,16,p+1,16,0);
		p=(u8*)strstr((const char*)(p2+1),"\"");
		p1=(u8*)strstr((const char*)(p+1),"\"");
		p2=p1;
		*p1=0;		
		Show_Str(x,y+48,240,16,(u8 *)"密码:",16,0);Show_Str(x+40,y+48,240,16,p+1,16,0);
		p=(u8*)strstr((const char*)(p2+1),",");
		p1=(u8*)strstr((const char*)(p+1),",");
		*p1=0;
		Show_Str(x,y+64,240,16,(u8 *)"通道号:",16,0);Show_Str(x+56,y+64,240,16,p+1,16,0);
		Show_Str(x,y+80,240,16,(u8 *)"加密方式:",16,0);Show_Str(x+72,y+80,240,16,(u8*)ATK_ESP8266_ECN_TBL[*(p1+1)-'0'],16,0);
	}
	myfree(SRAMIN,p);		//释放内存 
	myfree(SRAMIN,p1);		//释放内存 
	myfree(SRAMIN,p2);		//释放内存 
...
{ 
	...
	Show_Str(x,y+20,240,12,(u8 *)"SSID:",12,0);
	Show_Str(x,y+36,240,12,(u8 *)"加密方式:",12,0);
	Show_Str(x,y+52,240,12,(u8 *)"密码:",12,0); 
	POINT_COLOR=BLUE;
...
u8 atk_8266_netpro_sel(u16 x,u16 y,u8* name) 
{
	...
	Show_Str(x,y+30,200,16,(u8 *)"KEY0:下一个",16,0); 				    	 
	Show_Str(x,y+50,200,16,(u8 *)"KEY1:上一个",16,0);				    	 
	Show_Str(x,y+70,200,16,(u8 *)"WK_UP:确定",16,0); 
	Show_Str(x,y+100,200,16,(u8 *)"请选择:",16,0); 
	POINT_COLOR=BLUE;
 	Show_Str(x+16,y+120,200,16,(u8 *)"TCP服务器",16,0); 				    	 
	Show_Str(x+16,y+140,200,16,(u8 *)"TCP客户端",16,0);				    	 
	Show_Str(x+16,y+160,200,16,(u8 *)"UDP模式",16,0); 
	POINT_COLOR=RED;
	Show_Str(x,y+120,200,16,(u8 *)"→",16,0); 
	while(0)
	{
		key=KEY_Scan(0); 
		if(key)
		{
			...
			Show_Str(x,y+120+netpro*20,200,16,(u8 *)"  ",16,0);//清空之前的显示
			...
			Show_Str(x,y+120+netpro*20,200,16,(u8 *)"→",16,0);//指向新条目
...
PRESTA:		
	...
	{
		LCD_Clear(WHITE);
		if(atk_8266_ip_set((u8 *)"WIFI-AP远端UDP IP设置","UDP模式",(u8*)portnum,ipbuf))goto PRESTA;	//IP输入
...
//	u16 rlen=0;
	...
	u8 timex;
	POINT_COLOR=RED;
	Show_Str_Mid(0,30,(u8 *)"ATK-ESP8266 WIFI模块测试",16,240); 
	while(atk_8266_send_cmd((u8 *)"AT",(u8 *)"OK",20))//检查WIFI模块是否在线
	{
		...
		atk_8266_mtest_ui(32,30); //ATK-ESP8266模块测试主函数
	while(1)
	{
		delay_ms(10); 
		...
		if(key)
		{
			...
			{
				case 1://KEY0
					Show_Str_Mid(0,30,(u8 *)"ATK-ESP WIFI-AP+STA 测试",16,240);
					Show_Str_Mid(0,50,(u8 *)"正在配置ATK-ESP8266模块，请稍等...",12,20);
